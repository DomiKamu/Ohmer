//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// THIS MODULE IS ASSUMED AS "DEPRECATED" AND DON'T BE USED ANYMORE AS NEW INSTANCE!                      ////
// PLEASE REPLACE ALL INSTANCES IN YOUR PATCHES BY "KLOKSPID MKII" (FROM OHMERPREMS PLUGIN), INSTEAD!     ////
...
struct KlokSpidTitaniumSignatureMenu : MenuItem {
	KlokSpidModule *module;
	void onAction(const event::Action &e) override {
		module->Theme = 5; // Model: Titanium Signature.
		module->portMetal = 1; // Golden connectors.
	}
};

struct KlokSpidThemeMenuItems : MenuItem {
	KlokSpidModule *module;
	Menu *createChildMenu() override {
		...
		klokspidcreamymenu->rightText = CHECKMARK(module->Theme == 0);
		...
		klokspidstagerepromenu->rightText = CHECKMARK(module->Theme == 1);
		...
		klokspidabsolutenightmenu->rightText = CHECKMARK(module->Theme == 2);
		...
		klokspiddarksignaturemenu->rightText = CHECKMARK(module->Theme == 3);
		...
		klokspiddeepbluesignaturemenu->rightText = CHECKMARK(module->Theme == 4);
		...
		klokspidtitaniumsignaturemenu->rightText = CHECKMARK(module->Theme == 5);
...
struct KlokSpidWidget : ModuleWidget {
	...
	KlokSpidWidget(KlokSpidModule *module) {
		...
		addParam(createParam<Ohmer_Encoder>(Vec(20, 106), module, KlokSpidModule::PARAM_ENCODER));
		// Push button (gold), used to toggle START/STOP, also used to enter SETUP, and to advance to next parameter in SETUP.
		buttonGold = createParam<Ohmer_ButtonGold>(Vec(94, 178), module, KlokSpidModule::PARAM_BUTTON);
		addParam(buttonGold);
		// Push button (silver), used to toggle START/STOP, also used to enter SETUP, and to advance to next parameter in SETUP.
		buttonSilver = createParam<Ohmer_ButtonSilver>(Vec(94, 178), module, KlokSpidModule::PARAM_BUTTON);
	...
	void step() override {
		...
		if (!module) {
			...
			if (rack::settings::preferDarkPanels)
				setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Absolute_Night.svg")));
				else setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Creamy.svg")));
			...
			// Silver momentary button is used as default (Creamy or Absolute Night panel).
		...
		if (module->Theme != module->prevTheme) {
			...
			switch (module->Theme) {
				case 0:
					// Creamy.
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Creamy.svg")));
					break;
				case 1:
					// Stage Repro.
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Stage_Repro.svg")));
					break;
				case 2:
					// Absolute Night.
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Absolute_Night.svg")));
				...
				case 3:
					// Dark "Signature".
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Dark_Signature.svg")));
					break;
				case 4:
					// Deepblue "Signature".
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Deepblue_Signature.svg")));
					break;
				case 5:
					// Titanium "Signature".
					setPanel(createPanel(asset::plugin(pluginInstance, "res/KlokSpid_Titanium_Signature.svg")));
			}
			...
			if (module->portMetal == 1) {
				...
				// Momentary button: golden is visible...
				buttonGold->visible = true;
				// ...silver is hidden.
				buttonSilver->visible = false;
			}
			else {
				...
				bottomLeftScrewGold->visible = false;
				bottomRightScrewGold->visible = false;
				// Momentary button: silver is visible...
				buttonSilver->visible = true;
				// ...golden is hidden.
				buttonGold->visible = false;
			}
	...
	void appendContextMenu(Menu *menu) override {
		...
		KlokSpidThemeMenuItems *klokspidthememenuitems = new KlokSpidThemeMenuItems;
		klokspidthememenuitems->text = "Model";
		klokspidthememenuitems->rightText = RIGHT_ARROW;
		klokspidthememenuitems->module = module;
		menu->addChild(klokspidthememenuitems);
	}
